...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, bool a;bool *tmp=&a; s->dnpc = isa_raise_intr(isa_reg_str2val("a7",tmp),s->pc);); 
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", carrw  , C, R(rd) = *(csr(imm)); *(csr(imm)) = src1);
