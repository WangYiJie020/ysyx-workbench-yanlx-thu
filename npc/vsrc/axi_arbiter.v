module axi_arbiter(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if(bus_busy == 0 ) begin
                ...
                if(arvalid_i_b == 1  && ar_switch==0) begin 
                    ar_switch <= 1; r_switch <= 1;
                end
                else if(arvalid_i_b == 1&& ar_switch==0)begin
                    arready_o_b <= 0;
                end
    ...
    assign araddr_o = (bus_busy == 1)? 0: 
        ((arvalid_i_b==1 && ar_switch==0)?araddr_i_b : araddr_i_a);
    
    assign arid_o = (bus_busy == 1)? 0: 
        ((arvalid_i_b==1 && ar_switch==0)?arid_i_b : arid_i_a);
    
    assign arlen_o = (bus_busy == 1)? 0: 
        ((arvalid_i_b==1 && ar_switch==0)?arlen_i_b : arlen_i_a);
    
    assign arsize_o = (bus_busy == 1)? 0: 
        ((arvalid_i_b==1 && ar_switch==0)?arsize_i_b : arsize_i_a);
    
    assign arburst_o = (bus_busy == 1)? 0: 
        ((arvalid_i_b==1 && ar_switch==0)?arburst_i_b : arburst_i_a);
    /*
    always@(*) begin
        case(ar_switch)
            1: begin
